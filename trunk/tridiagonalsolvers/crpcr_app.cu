...
void crpcr(T *a, T *b, T *c, T *d, T *x, int systemSize, int numSystems, int restSystemSize)
{
	...
	for(int i = 0;i<numIterations;i++)
		crpcrKernel<<< grid, threads,(systemSize+1)*5*sizeof(T)+restSystemSize*(5+0)*sizeof(T)>>>(d_a, d_b, d_c, d_d, d_x, restSystemSize);

	cudaThreadSynchronize();
    cutStopTimer(timer);
	printf("crpcr: numSystems: %d, systemSize: %d, GPU kernel time: %f ms\n", numSystems, systemSize, cutGetTimerValue(timer)/numIterations);
